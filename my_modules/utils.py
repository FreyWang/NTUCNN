...
def load_state_dict(model, pretrain_path, state_dir, prefix):
    #model.initialize()  # initialize new layers if necessary, or do other stuffs, or you can initialize in module
    ...
    if latest_state != -1:
        ...
        for name, param in state_dict.items():
            ...
            if param.size() != model_state[name].size():
                ...
                continue
            if isinstance(param, (nn.Parameter, Variable)):  # param is nn.Parameter, OR Variable
                # backwards compatibility for serialized parameters
                param = param.data  #  tensor
...
def submodule_params(model, submodule_str):
    ...
    # return a list,e.g   'classifier.3' ==> ['classifier', '3']

    if len(children) == 1 and \
       isinstance(getattr(model, submodule_str), nn.Parameter):
        return [(submodule_str, getattr(model, submodule_str))]
        # in fact , model.layer is nn.modules, must not be parameter, so it is always false

    module = model

    for child in children:
        module = getattr(module, child)  # model.classifier.3
...
def get_param_groups(model, config):
    ...
    for cfg_param_group in config.PARAM_GROUPS:
        if cfg_param_group['params'][0] == 'default':
            default = cfg_param_group.copy()  # dict
            continue  # jump to fc

        module_names = cfg_param_group['params']  # ['fc']
        named_params = []
        for module_name in module_names:  # 'fc'
            named_params.extend(submodule_params(model, module_name))  # [('fc.weight', tensor), ('fc.bias', tensor)]
        optional_var_names.extend([e[0] for e in named_params])  # ['fc.weight', 'fc.bias']
        param_group = cfg_param_group.copy()
        param_group['params'] = [e[1] for e in named_params]  # [weight tensor, weight tensor]
